...
<form id="myForm" action="{{ $accion == 'E' ? route('Grupo.update', $grupo->idGrupo) : route('Grupo.store') }}" method="POST">
    @csrf
    @if($accion == 'E')
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @for ($i = 7; $i <= 21; $i++)
                        <tr>
                            <td>{{ $i }}-{{ $i + 1 }}</td>
                            <td><input type="checkbox" name="horarios[]" value="L,{{ $i }}" 
                           {{ in_array("L,{$i}", $existingHorarios) ? 'checked' : '' }}></td>
                <td><input type="checkbox" name="horarios[]" value="M,{{ $i }}" 
                           {{ in_array("M,{$i}", $existingHorarios) ? 'checked' : '' }}></td>
                <td><input type="checkbox" name="horarios[]" value="X,{{ $i }}" 
                           {{ in_array("X,{$i}", $existingHorarios) ? 'checked' : '' }}></td>
                <td><input type="checkbox" name="horarios[]" value="J,{{ $i }}" 
                           {{ in_array("J,{$i}", $existingHorarios) ? 'checked' : '' }}></td>
                <td><input type="checkbox" name="horarios[]" value="V,{{ $i }}" 
                           {{ in_array("V,{$i}", $existingHorarios) ? 'checked' : '' }}></td>
                        </tr>
                    @endfor
                </tbody>
            </table>
...
function toggleHorario() {
    ...
    const form = document.getElementById("myForm"); // Suponiendo que el formulario tiene id="grupoForm"
    ...
    if (edificioYHorario.style.display === "none") {
        ...
        toggleButton.type = "submit"; // Cambiar tipo a submit
        ...
        document.getElementById("idMateria").readOnly = true;
        document.getElementById("idPersonal").readOnly = true;
        ...
        if (toggleButton.textContent === "Terminar Horario") {
            ...
            // Reactivar los campos originales
            ...
            document.getElementById("idMateria").readOnly = false;
            document.getElementById("idPersonal").readOnly = false;
